...
- Phase 4B本轮支线为 `test0.5`，从已验收的 `test0.4` 继续；当前等待远程跨平台门禁。
...
| Phase 4 MQTT控制/TCP回退/WebSocket兼容 | 进行中；Phase 4A上行与Phase 4B只读双向验证本地通过 |
...
| 自动测试 | 53通过；Ruff与mypy strict通过；Phase 4B远程矩阵待跑 |
| 硬件验证 | 未运行 |
...
- Phase 4B本地验收已通过；等待macOS/Windows GitHub Actions、`test0.5`最终提交/push及远程哈希核对。
