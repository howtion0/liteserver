...
`0.4.3` 使用真实loopback TCP、HTTP/WebSocket和内嵌MQTT Broker配合fake设备验证协议、鉴权、隔离、动作/stop生命周期和资源释放。Phase 4E又在EVA1/EVA2固件`2.0.6`上验证了真实MQTT hello/heartbeat、14动作查询、action/stop、6步walk、双机隔离、重复ID和Server/Broker重启恢复。`test0.9`进一步在EVA1固件`2.0.11`上完成MQTT信令、AES-CTR UDP Opus、ASR/TTS、循环WakeGate和单设备LLM工具链；`0.5.0/test1.0`增加Server端显式多设备批量控制、每设备对话投影和正式会话start/stop，并让2.0.16正式MQTT连接复用显式mDNS解析。当前EVA1/EVA2/EVA3均以2.0.16和独立身份在线，三机正式批量动作请求3、接受3、失败0且分别完成；EVA2另完成一次单设备MQTT+UDP问答烟测。连续Opus仍不走Message Bus或MQTT控制Topic，协议边界未改变。尚未完成：

- 多设备并发语音/工具会话隔离；EVA2单会话成功不能替代该门禁。
