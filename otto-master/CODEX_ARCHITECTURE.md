...
- 维护按device_id/session隔离的脱敏对话投影，供刷新和事件重连恢复用户文字、助手句子、工具结果与状态；该快照与事件流服从当前控制台访问策略，不能进入无需接入Server即可读取的静态资源或日志。
...
- `test1.2`默认采用可信家庭局域网直接控制：同源WebUI无需令牌即可调用控制API，但Origin仍只允许显式白名单、环回/私网IP和`.local`同主机来源。可选安全模式由`server.console_auth_required=true`显式开启，此时无令牌或401必须锁定动作/对话控件并显示失败原因。正式Runtime固定使用HTTP 8081，旧8080诊断进程不得与其并存造成入口歧义。
...
- 新控制台必须适配Otto已经冻结的真实字段、当前访问策略、批量结果和事件语义，禁止为了兼容参考占位后端而伪造成功或改变设备领域合同。在线设备动作目录为空时只允许自动执行既有只读verify并重新取目录，不能借目录恢复自动下发动作。
...
- 所有知乎及画像API都服从现有控制台访问策略；默认直控模式仅适用于可信家庭局域网，可选安全模式启用后必须携带控制令牌。画像可作为普通非密钥设置写入SQLite；查询结果只保存在有界内存事件中。
