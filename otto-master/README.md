...
当前为 **Phase 4C / 0.4.2 本地验收通过、远程矩阵待验收**：已接入Master MQTT Gateway、Device Session、只读连接验证、持久命令仓库和每设备Dispatcher。两个fake设备经真实内嵌Broker完成精确动作/stop下发、`requested → published → accepted → moving → completed`、隔离和重复ID去重。TCP/WebSocket兼容、固件MQTT缺口、语音云服务和EVA真机验收仍属于后续检查点。
